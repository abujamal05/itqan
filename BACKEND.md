...
**Currency.** Paddle does not support OMR, so Itqan publishes **2.9 OMR** and
charges **$7.50**. The Paddle price is created in USD at 7.50. Decided
2026-08-24 and not open.

An earlier version of this paragraph said the peg meant "the two cannot drift",
and the arithmetic does not support that. The peg is fixed at
1 OMR = 2.6008 USD, so 2.9 OMR converts to **$7.5423**, and $7.50 is **2.8837
OMR**. The rate does not move; the two published figures are simply not the same
number, and $7.50 is the rounder one, landing a few baisa in the customer's
favour. Say both figures, never that one equals the other, and never explain the
conversion on a page a user reads.
